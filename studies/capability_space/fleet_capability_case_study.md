...
subtitle: "Study I-F — 9 actuator faults applied to every spacecraft in the fleet, and what the spacecraft classes have in common"
...
actuation-envelope fleet — 18 vehicles with usable actuator
geometry, across 5 classes — and then compared between vehicles
and between classes.
...
| Fault | Tier | Applied to |
|:------------------------------|:----------|:----------------------------------------|
| Engine out | engine | one unit of that tier, swept over every unit |
| Thrust reduction, $\eta$=0.15 | engine | one unit of that tier, swept over every unit |
| Thrust reduction, $\eta$=0.50 | engine | one unit of that tier, swept over every unit |
| Thrust excess, $\eta$=1.30 | engine | one unit of that tier, swept over every unit |
| TVC misalignment (3°, 2°) | engine | one unit of that tier, swept over every unit |
| Gimbal seizure | engine | one unit of that tier, swept over every unit |
| Valve stuck open (30 % floor) | engine | one unit of that tier, swept over every unit |
| RCS thruster dead | rcs | one unit of that tier, swept over every unit |
| RCS thruster stuck open | rcs | one unit of that tier, swept over every unit |


Two conventions are worth stating, because they are choices rather than
readings:

* **`valve stuck open` is 30% of the affected unit's own
...
![Moment authority retained after the worst single failure](/home/omersayilgan/Desktop/ThesisGit/studies/capability_space/figures/IF1_fleet_map.png)
...
| Fault | Vehicles exposed | Mean retained | Range | Lost zero-moment |
|:------------------------------|:----------------|:--------------|:------------------|:----------------|
| Gimbal seizure | 8 | 0.28× | 0.00× – 0.93× | 0 |
| Engine out | 14 | 0.49× | 0.00× – 1.00× | 0 |
| Thrust reduction, $\eta$=0.15 | 14 | 0.56× | 0.15× – 1.00× | 0 |
| Thrust reduction, $\eta$=0.50 | 14 | 0.74× | 0.50× – 1.00× | 0 |
| RCS thruster stuck open | 12 | 0.95× | 0.87× – 1.00× | 0 |
| RCS thruster dead | 12 | 0.95× | 0.87× – 1.00× | 0 |
| Valve stuck open (30 % floor) | 14 | 0.97× | 0.87× – 1.00× | 0 |
| TVC misalignment (3°, 2°) | 14 | 1.01× | 0.94× – 1.35× | 0 |
| Thrust excess, $\eta$=1.30 | 14 | 1.15× | 1.00× – 1.30× | 0 |


**Gimbal seizure** is the most damaging fault on average
(0.28× of healthy moment volume), and
**Engine out** is the most variable — its effect ranges from
0.00× to 1.00× depending
...
![Fault response by spacecraft class](/home/omersayilgan/Desktop/ThesisGit/studies/capability_space/figures/IF2_by_category.png)


| Class | Engine out | Thrust reduction | Thrust reduction | Thrust excess | TVC misalignment | Gimbal seizure | Valve stuck open | RCS thruster dead | RCS thruster stuck open |
|---|---|---|---|---|---|---|---|---|---|
| Boosters | 0.17× | 0.29× | 0.58× | 1.25× | 1.00× | 0.18× | 1.00× | — | — |
| LEO Satellites | 0.79× | 0.82× | 0.89× | 1.06× | 0.99× | — | 0.94× | 0.89× | 0.89× |
| GEO Satellites | 0.78× | 0.81× | 0.89× | 1.07× | 1.16× | — | 0.93× | 0.93× | 0.93× |
| Crewed Vehicles | 0.37× | 0.47× | 0.69× | 1.19× | 0.99× | 0.20× | 0.99× | 0.99× | 0.99× |
| Deep Space Probes | 0.77× | 0.81× | 0.89× | 1.07× | 0.97× | 0.93× | 0.95× | 0.96× | 0.96× |


## The quantitative answer


![Distance between fault responses, and class membership](/home/omersayilgan/Desktop/ThesisGit/studies/capability_space/figures/IF3_similarity.png)
...
| Pairing | Mean distance | Pairs |
|:------------------------|:----------------|:----------|
| Same spacecraft class | 0.130 | 21 |
| Different classes | 0.187 | 100 |


| Class | Vehicles compared | Mean distance within the class |
|:----------------------|:------------------|:------------------------------|
| Boosters | 4 | 0.177 |
| LEO Satellites | 3 | 0.032 |
| GEO Satellites | 2 | 0.237 |
| Crewed Vehicles | 4 | 0.140 |
| Deep Space Probes | 4 | 0.099 |


The classes are not equally coherent. LEO Satellites, Deep Space Probes
hold together more tightly than the fleet
average,  while GEO Satellites is more spread out than two vehicles picked at random.


**Same-class vehicles do respond more alike** — 0.130 against
0.187, a gap of 0.057. Mission class is a partial proxy
...
**1. Tier count decides exposure.** 8 vehicles carry a single main
...
Those are: Apollo CSM, Apollo LM, Ariane 5 ECA, Juno, Orion, SDO, Vega, Vega-C.


By contrast Crew Dragon, Delta IV Heavy, Europa Clipper, New Horizons carry three or more, and
...
![The worst single failure per class](/home/omersayilgan/Desktop/ThesisGit/studies/capability_space/figures/IF4_exemplars.png)
...
Engine out costs one vehicle 0.00× and another
1.00×. Any number quoted from a single-vehicle
...
* **One operating point is missing.** Study I could condition its moment sets on
  ...
  Study I showed.
* **0 vehicles have no usable actuator
